...
- Built with LangChain (PromptTemplate + a pluggable chat-model chain) so an
  actual LLM (Anthropic/OpenAI/etc.) can be dropped in via LANGCHAIN_LLM_
  PROVIDER / an API key once the team decides which one to use and has a
  key to spend.
- No API key is available in this environment, so generate_summary() falls
  back to a deterministic, template-based summary (_fallback_summary) built
  from the same structured context the LLM prompt would receive. This keeps
  the module fully testable offline and keeps app behaviour identical
...
def generate_summary(candidate_id: int, exam_id: int, llm: Optional[Any] = None) -> Dict[str, Any]:
    ...
    it's used with REPORT_PROMPT to generate the summary. Otherwise this
    falls back to a deterministic template summary built from the same
    context - see module docstring for why.
